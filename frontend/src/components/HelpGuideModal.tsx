...
export const HELP_GUIDE_DATA: HelpGuideItem[] = [
  ...
  },
  {
    ...
    tips: 'チェックON（推奨）の場合、元の予定はキャンセルとして最下部に残り、備考欄に自動で「〇/〇へ移動」が記録されます。移動先には元の備考を引き継いだ同一内容の新規フリー予定が作成されます。チェックOFFにすると日付のみが直接スライド移動します。',
    keywords: ['別日へ移動', '移動', '別日', '振替', 'スライド', '日程変更', 'キャンセル', '日付変更', '履歴', '延期', '日延べ', '備考'],
...
function matchGuidesByNaturalLanguage(input: string): { guides: HelpGuideItem[]; replyText: string } {
  ...
  if (filtered.length === 0) {
    ...
    if (cleanInput.includes('予定') && (cleanInput.includes('入れる') || cleanInput.includes('追加') || cleanInput.includes('登録'))) {
      ...
      reply = '予定の別日への移動・振替ですね！右クリックメニューまたは複数選択時の「別日へ移動」から、元の予定の備考に「〇/〇へ移動」と自動記録してキャンセル履歴を残しつつ、新しい日付へスムーズに振替できます：';
