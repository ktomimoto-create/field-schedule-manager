...
## [2026-09-11] 予定追加・編集サイドバーの文言添削・同行者プルダウン複数選択・区分FTSデフォルト化

### 変更の目的
1. **カード見出し文言の添削・簡潔化**:
   - カード1: 「1. 案件特定（番号入力で下記が一瞬で自動反映）」 ➔ **「1. 物件情報」**
   - カード2: 「2. 配車・日程決定」 ➔ **「2. 日程・対応者」**
   - カード3: 「3. 作業内容・TIME・備考」 ➔ **「3. 作業内容」**
   - カード1上部の連携バッジ（`⚡ FC / 物件マスタ自動連携`）を削除し、より洗練された静かなデザインへ統一。
2. **例文（プレースホルダー）の全削除**:
   - 依頼番号、号機、物件名、BOX数、タイプ、エリア、県別、時間、コース、移動手段、同行者、TIME、作業内容、備考の各入力欄から不要な例文テキスト（「例: ...」等）をすべて削除し、入力欄をすっきり見やすく整理。
3. **同行者のプルダウン複数選択＋フリー入力両対応**:
   - マスタ登録メンバーをプルダウンからワンクリックで追加・解除できる `<select>` を新設（選択中メンバーには `✓` 表示）。
   - テキスト入力欄への直接キーボード入力も可能とし、マスタ未登録者（他部署・応援・協力会社メンバー等）の同行にも柔軟に対応。
4. **区分のデフォルト値を「FTS」に設定**:
   - 予定新規追加時の区分の初期値を「FTS」に変更。コース未入力時にもデフォルト「FTS」を維持するよう制御を最適化。
...
- **[MODIFY] [ScheduleModal.tsx](file:///C:/Users/000644/.gemini/antigravity/scratch/field-schedule-manager/frontend/src/components/ScheduleModal.tsx)**:
  - 各カードタイトル変更、バッジ削除、例文プレースホルダー削除。
  - 同行者に複数選択プルダウン `<select id="co_worker_select">` を追加し、直接テキスト入力との両立を実装。
  - `division` のデフォルト値を `FTS` に設定、プルダウン選択肢の先頭を `FTS` に変更。
- **[MODIFY] [specification.md](file:///C:/Users/000644/.gemini/antigravity/scratch/field-schedule-manager/specification.md)**:
  - 第17章を最新のカード名・同行者仕様・区分仕様に同期更新。

---

## [2026-09-11] 予定追加・編集サイドバーの案A改訂版（スマートカード型・自動補完最優先）レイアウト刷新

### 変更の目的
1. **自動補完起点への動線整理（ファーストビューでの連携完了）**:
   - FC依頼番号（11桁）および号機による物件・住所・BOX・タイプ等の自動補完機能が導入されたことに伴い、最上部に「カード1: 案件特定」を配置。
   - 依頼番号または号機を入力してフォーカスアウトすると、物件名・BOX数・タイプ・エリア・県別が一瞬で自動補完される爽快なUXを実現しました。
   - 補完完了時にはカード全体が淡いパープル色で1.1秒間滑らかにフラッシュ（`autofill-flash`）し、自動入力されたことが直感的に伝わります。
2. **ユーザーフィードバックに基づく各種項目の最適化**:
   - **時間定型チップ**: `[ 必ず ]`（赤色バッジ）、`[ AM ]`、`[ PM ]` の3種のみに厳選し、「※ピンポイント時刻は手入力...」の注釈文字を非表示化。
   - **エリア・県別**: 独立した2項目として配置し、`addressResolver.ts` の定義（東京都練馬区 ➔ エリア: `練馬区` / 県別: `23`、埼玉県さいたま市南区 ➔ エリア: `南区S` / 県別: `埼玉`）に100%準拠。
   - **TIME**: （期限）表記を廃止し、作業管理者が予測する作業目安時間として「TIME」に改名・再配置。
   - **配車情報の集約**: コース、区分（セレクト式: 委託/FTS/未定/直行直帰）、移動手段を「カード2: 配車・日程決定」に集約。
3. **表示幅の最適化**:
   - サイドバーの横幅を 480px から 520px（`max-width: 95vw`）へ微調整し、2列〜4列のグリッドが窮屈にならず視認性を大幅に向上。
...
- **[MODIFY] [ScheduleModal.tsx](file:///C:/Users/000644/.gemini/antigravity/scratch/field-schedule-manager/frontend/src/components/ScheduleModal.tsx)**:
  - 4つのスマートカード構造に再構築：
    - **カード1: 案件特定**: 依頼番号(11桁) & 号機（2列） ➔ 物件名（全幅） ➔ BOX数、タイプ、エリア、県別（4列）。
    - **カード2: 配車・日程決定**: 対応予定日 & 時間（定型チップ `必ず` / `AM` / `PM`） ➔ 対応者 & 同行者（連動登録） ➔ コース、区分、移動手段（3列）。
    - **カード3: 作業内容・TIME・備考**: 種別 & TIME（作業目安時間） ➔ 作業内容 ➔ 備考。
    - **カード4: 管理情報**: 障害区分、level、level 2（3列） ➔ 登録者・最終更新者情報。
  - `isAutofillFlashing` ステートと補完成功時のトリガーを追加。
  - `division`（区分）の手動セレクト操作を有効化。

#### 2. スタイルシート
- **[MODIFY] [index.css](file:///C:/Users/000644/.gemini/antigravity/scratch/field-schedule-manager/frontend/src/index.css)**:
  - `.schedule-sidebar-content` の幅を `520px`（`max-width: 95vw`）へ調整。
  - `.schedule-modal-card`、`.schedule-card-header`、`.schedule-card-title`、`.schedule-card-badge`、`.autofill-flash`、`.schedule-time-chip` 等のスマートカード用CSSを新設。

#### 3. 仕様書
- **[MODIFY] [specification.md](file:///C:/Users/000644/.gemini/antigravity/scratch/field-schedule-manager/specification.md)**:
  - 第17章「予定追加・編集サイドバー レイアウト刷新仕様（案A改訂版：スマートカード型）」を正式追記・更新。
...
### 変更の目的
1. **別日移動時における移動先追跡・経緯の明確化**:
   - 月間予定表で予定を「別日へ移動」した際、元の予定がキャンセルとして最下部に履歴保持されるものの、備考欄に移動先が記録されておらず「何日に振り替えられたのか」がひと目で分からない状態でした。
   - スプレッドシート運用と同様に、元の予定の備考欄へ**「〇/〇へ移動」**（例: `9/15へ移動`）を自動付与することで、キャンセルされた予定がいつに移動したのかを一目で追跡できるように改善しました。
2. **元の備考の保護・移動先予定への確実な引き継ぎ**:
   - 元の予定に「鍵預かり」「脚立必要」などの業務備考が入っていた場合、移動先の新規予定へ備考が確実に引き継がれるよう改修しました。
   - 元の予定の備考には、既存メモを壊さず先頭に `9/15へ移動 / 既存メモ` の形式で記録され、再移動時にも古い移動先が最新の日付へ綺麗に更新されます。
...
- **[MODIFY] [CalendarView.tsx](file:///C:/Users/000644/.gemini/antigravity/scratch/field-schedule-manager/frontend/src/components/CalendarView.tsx)**:
  - `handleExecuteMove`:
    - 移動先の日付（`destinationDate`）から `M/Dへ移動`（年跨ぎ時は `YYYY/M/Dへ移動`）を生成。
    - 元の予定の備考から正規表現で古い移動履歴を除去した `cleanNotes` を作成。
    - 元のキャンセル予定の `notes` に `${formattedDestDate} / ${cleanNotes}`（または単体）を設定して更新。
    - 移動先の日付に作成される新規フリー予定の `notes` に `cleanNotes` を確実に引き継ぎ。
  - ダイアログ説明文の更新:
    - 「※ 元の予定はステータス「キャンセル」となり、備考欄に「〇/〇へ移動」が自動記録されます。」と明記。

#### 2. 操作ガイド・ヘルプモーダル
- **[MODIFY] [HelpGuideModal.tsx](file:///C:/Users/000644/.gemini/antigravity/scratch/field-schedule-manager/frontend/src/components/HelpGuideModal.tsx)**:
  - 「予定をキャンセルして別の日付に移動（振替）したい」のTipsおよびAIチャット返答に、備考欄への自動記録について追記。

#### 3. 仕様書
- **[MODIFY] [specification.md](file:///C:/Users/000644/.gemini/antigravity/scratch/field-schedule-manager/specification.md)**:
  - 第4.11章「別日へ移動（振替）と元の予定備考への移動先自動記録仕様」を新設。

---

## [2026-09-10] 月間予定表における同日複数予定の表示仕様（独立表示維持）の確認・仕様書反映

### 検討と決定の背景
1. **同日複数予定の視認性に関する検討**:
   - 1人のスタッフが同日に複数の現場予定を持つ場合、全行にアバター・名前・コース番号を表示することが煩わしく感じられないか検討を行いました。
   - 案として「同上（〃）ツリー表示」や「セル結合風グループ化」などのデザインパターンを検討。
2. **現状維持の決定とメリット**:
   - 行ごとの直接インライン編集やコンテキストメニュー操作のしやすさ、セル範囲選択・コピペ（Ctrl+C / Ctrl+V）時のデータ完全性、および担当者名検索（Ctrl+F）時の全行ハイライトの確実性を優先し、**「各行にアバター・名前・コース番号を独立して表示する標準仕様を一旦そのまま維持する」**ことを決定しました。
   - 今後の運用や現場からのフィードバックに応じて、必要であれば表示切り替えオプション等の導入を柔軟に行えるよう、仕様書（`specification.md` 第4.10章）に方針を明文化しました。
...
### 変更の目的
1. **比較軸の適正化（開発時不具合の混入排除）**:
   これまでの資料内で、開発・実装途中における内部仕様や不具合の改善（生JSONログの改修、開発中の特定スタッフの表示崩れ、ボタン配色と行色の不一致など）が一部BEFOREとして混ざってしまっていた記述を全面的に是正。社内配布・説明資料としてふさわしく、**「現在行われているスプレッドシート運用における現場の悩み・課題（BEFORE）」 ➔ 「新システム導入による解決・改善（AFTER）」**の対比に11項目すべてを統一しました。
2. **現場・業務視点の課題明瞭化**:
   - セルごとの個別手動色塗り・書式変更
   - 切り取り移動による過去経緯の消失
   - スタッフ表と見比べながらの手動転記・コース割り振り
   - セル内改行や行挿入による表レイアウトの崩れ
   - 共有シートの編集や印刷範囲の個別設定負担
   - 表記ゆれ（フルネーム・苗字）による文字切れ
   - スプレッドシート履歴からの変更原因の追跡困難
   - 運用ルールの属人化と管理者への質問集中
   - 手動色塗りによる配色のばらつき
   - 休職・退職スタッフ増減時の手動メンテと過去データ崩れ
   - 社員名簿を見ながらの手入力・転記作業
   上記のように、現場が直面している実運用上のペインをBEFOREとして明確に定義しました。

### 変更内容

#### 1. リリース説明資料（HTML）
- **[MODIFY] [release_guide.html](file:///C:/Users/000644/.gemini/antigravity/scratch/field-schedule-manager/release_guide.html) / [frontend/public/release_guide.html](file:///C:/Users/000644/.gemini/antigravity/scratch/field-schedule-manager/frontend/public/release_guide.html)**:
  - 改善点11項目すべてのBEFOREを「従来（スプレッドシート運用）」として再定義・全面改稿。
  - システム開発中の内部事情（JSONコード、平本・河部の文字切れ、ボタン配色不一致など）を完全排除。

#### 2. 仕様書（specification.md）
- **[MODIFY] [specification.md](file:///C:/Users/000644/.gemini/antigravity/scratch/field-schedule-manager/specification.md)**:
  - 第13.2章の項目名称を最新のスプレッドシート運用対比版に同期。

---

## [2026-09-10] リリース説明資料（HTML）の表現適正化（誇張表現の抑制・具体名削除）

### 変更の目的
1. **ビジネス向けドキュメントとしてのトーン＆マナー適正化**:
   リリース説明資料（`release_guide.html`）内に含まれていた「劇的」「一発適用！」「完全自動」「完全ゼロ」「解読不能」などの過度・誇張な表現を排除し、業務報告・社内配布資料としてふさわしい落ち着いた客観的なビジネス表現へ平準化しました。
2. **特定個人名表記の排除（一般化）**:
   「06. Excel出力・印刷時の苗字統一」の新システム説明において、具体名（「平本」「河部」等）が例示されていた箇所を削除し、苗字のみを抽出する一般的な仕様説明に改めました。
3. **資料と仕様書の整合性維持**:
   改善点項目数（11項目）や各見出し・説明文の変更内容を、HTML資料、フロントエンド公開用ファイル（`frontend/public/release_guide.html`）、仕様書（`specification.md`）へ完全同期しました。

### 変更内容

#### 1. リリース説明資料（HTML）
- **[MODIFY] [release_guide.html](file:///C:/Users/000644/.gemini/antigravity/scratch/field-schedule-manager/release_guide.html) / [frontend/public/release_guide.html](file:///C:/Users/000644/.gemini/antigravity/scratch/field-schedule-manager/frontend/public/release_guide.html)**:
  - ヘッダー・ナビゲーション・見出しから「劇的」を完全削除（例: 「現状からの劇的改善点」➔「従来運用からの主な改善点 11選」）。
  - 「06. Excel出力・印刷時の苗字統一」から「平本」「河部」の具体名を削除し、「システム内部では正確な本名マスタを保持しつつ、Excel出力や一覧画面では苗字のみを自動抽出して表示」に変更。
  - 感嘆符（！）の多用や「わずか3秒」「10秒で完了」「解読不能」「トラブルの原因に」などの誇張表現を、客観的で丁寧な業務改善表現に改修。
  - チートシート導入文を「覚えておくと業務スピードが2倍になる...」から「日常業務をよりスムーズに行うためのショートカットと色ルール」に変更。

#### 2. 仕様書（specification.md）
- **[MODIFY] [specification.md](file:///C:/Users/000644/.gemini/antigravity/scratch/field-schedule-manager/specification.md)**:
  - 第13.2章の構成要素を更新（「従来運用からの主な改善点 11選」への名称変更、管理・監査ログ画面の紹介反映）。
